			...
			*/
			...
			if ( false === $wpdb->update( $wpdb->prefix . 'anwpfl_matches', $update_data, [ 'match_id' => $game_id ] ) ) {
				if ( 'anwp_match' === get_post_type( $game_id ) ) {
					return new WP_Error( 'anwp_rest_error', 'Update Data Error - ID:' . absint( $game_id ) . ' - Data: ' . wp_json_encode( $update_data ), [ 'status' => 400 ] );
				}
			}
	...
	public function run_migrate_lineups_from_meta( WP_REST_Request $request ) {
		...
		foreach ( $game_ids as $game_id ) {
			...
			*/
			...
			if ( ! $wpdb->insert( $wpdb->anwpfl_lineups, $insert_data ) ) {
				...
				if ( absint( $maybe_saved ) === absint( $game_id ) ) {
					if ( ! $wpdb->update( $wpdb->anwpfl_lineups, $insert_data, [ 'match_id' => absint( $maybe_saved ) ] ) ) {
						return new WP_Error( 'anwp_rest_error', 'Update Data Error - ID:' . absint( $game_id ) . ' - Data: ' . wp_json_encode( $insert_data ), [ 'status' => 400 ] );
					}
				} elseif ( 'anwp_match' === get_post_type( $game_id ) ) {
					return new WP_Error( 'anwp_rest_error', 'Insert Data Error - ID:' . absint( $game_id ) . ' - Data: ' . wp_json_encode( $insert_data ), [ 'status' => 400 ] );
				}
			}
