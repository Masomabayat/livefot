...
class AnWPFL_Assets {
	...
	public function admin_enqueue_scripts() {
		...
		*/
		...
		|--------------------------------------------------------------------------
		*/
		wp_enqueue_script( 'anwp-select2', AnWP_Football_Leagues::url( 'vendor/select2/select2.full.min.js' ), [ 'jquery', 'jquery-ui-sortable' ], '4.0.13', false );
		...
		if ( in_array( $current_screen->id, $plugin_pages, true ) ) {
			...
			*/
			...
			wp_enqueue_script( 'vuejs', AnWP_Football_Leagues::url( 'vendor/vuejs/vue' . $suffix . '.js' ), [], '2.6.14', false );
			...
			*/
			...
			|--------------------------------------------------------------------------
			*/
			wp_enqueue_script( 'anwpfl_admin_vue', AnWP_Football_Leagues::url( 'admin/js/anwpfl-admin-vue.min.js' ), [ 'vuejs', 'anwpfl_admin', 'wp-color-picker' ], AnWP_Football_Leagues::VERSION, true );
			...
			|--------------------------------------------------------------------------
			*/
			$this->app_admin_scripts_loader( $current_screen->id );
		...
		|--------------------------------------------------------------------------
		*/
		wp_enqueue_style( 'jexcel-v4', AnWP_Football_Leagues::url( 'vendor/jexcel/jexcel.min.css' ), [], '4.13.4' );
		wp_enqueue_script( 'jexcel-v4', AnWP_Football_Leagues::url( 'vendor/jexcel/jexcel.min.js' ), [ 'jexcel-suites-v4' ], '4.13.4', true );
		wp_enqueue_style( 'jexcel-suites-v4', AnWP_Football_Leagues::url( 'vendor/jexcel/jsuites.css' ), [], '4.17.5' );
		wp_enqueue_script( 'jexcel-suites-v4', AnWP_Football_Leagues::url( 'vendor/jexcel/jsuites.js' ), [], '4.17.5', true );
	...
	/**
	 * Handle admin scripts loading process.
	 *
	 * @param $current_screen_id
	 *
	 * @return void
	 */
	public function app_admin_scripts_loader( $current_screen_id ) {

		wp_register_script( 'vuejs-fl-3', AnWP_Football_Leagues::url( 'vendor/vuejs/vue.runtime.global.prod.min.js' ), [], '3.3.7', false );

		$loader_data = [];
		$page_prefix = sanitize_title( _x( 'Football Leagues', 'admin menu title', 'anwp-football-leagues' ) );

		if ( $page_prefix . '_page_anwpfl-toolbox' === $current_screen_id || 'football-leagues_page_anwpfl-toolbox' === $current_screen_id ) {
			/*
			|--------------------------------------------------------------------------
			| API Optimizer
			|--------------------------------------------------------------------------
			*/
			$loader_data['src'] = AnWP_Football_Leagues::url( 'admin/js/app/toolbox.min.js' );

			wp_dequeue_script( 'vuejs' );
			wp_dequeue_script( 'anwpfl_admin_vue' );
		} elseif ( 'toplevel_page_anwp-settings-tools' === $current_screen_id ) {
			/*
			|--------------------------------------------------------------------------
			| Import/Export
			|--------------------------------------------------------------------------
			*/
			$loader_data['src'] = AnWP_Football_Leagues::url( 'admin/js/app/import-tool.min.js' );
		 ...
		 */
		$loader_data = apply_filters( 'anwpfl/admin/app_js_loader', $loader_data, $current_screen_id );
		...
		*/
		if ( is_array( $loader_data ) && ! empty( $loader_data['src'] ) ) {
			...
			wp_enqueue_script( 'fl-admin-vue', $script_data['src'], $script_data['deps'], AnWP_Football_Leagues::VERSION, $script_data['in_footer'] );
		}
	}
